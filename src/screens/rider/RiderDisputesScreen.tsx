...
export default function RiderDisputesScreen() {
  ...
  const fetchDisputes = useCallback(async () => {
    try {
      const response = await disputeService.getRiderDisputes();
      if (response.success && Array.isArray(response.data)) {
        setDisputes(response.data);
      }
    } catch (error) {
      console.error('Error fetching rider disputes:', error);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const handleDisputePress = (dispute: Dispute) => {
    navigation.navigate('RiderDisputeDetail', { 
  ...
  const renderDisputeCard = ({ item }: { item: Dispute }) => {
    ...
    const isDeliveryRelated = ['late_delivery', 'rider_issue', 'damaged_products'].includes(item.type);
          ...
          <View style={styles.disputeNumber}>
            ...
            {isDeliveryRelated && (
              <View style={[styles.deliveryBadge, { backgroundColor: '#FEE2E2' }]}>
                <Ionicons name="bicycle" size={10} color="#EF4444" />
              </View>
            )}
          </View>
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]} edges={['top']}>
      ...
      <View style={[styles.header, { borderBottomColor: colors.border }]}>
        ...
        <Text style={[styles.headerTitle, { color: colors.text }]}>Delivery Disputes</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      <View style={[styles.infoBanner, { backgroundColor: isDark ? 'rgba(59, 130, 246, 0.15)' : '#EFF6FF' }]}>
        ...
        <Text style={[styles.infoBannerText, { color: isDark ? '#93C5FD' : '#1E40AF' }]}>
          These are disputes filed about deliveries you handled. Respond promptly to maintain your rating.
        </Text>
      </View>
      ...
      {filteredDisputes.length === 0 ? (
        <View style={styles.emptyContainer}>
          ...
          <Text style={[styles.emptyText, { color: colors.textSecondary }]}>
            {filter === 'all'
              ? "Great! No customers have filed disputes about your deliveries."
              : filter === 'active'
